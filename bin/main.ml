...
let encode list = List.map (fun lst -> List.length lst, List.hd lst) (pack list)
...
let decode list =
  let rec unwrap out len x = if len = 0 then out else unwrap (out @ [ x ]) (len - 1) x in
  let rec aux out = function
    | [] -> out
...
let drop list n =
  let rec maybe_remove idx n hd out = if idx mod n = 0 then out else out @ [ hd ] in
...
let split list n =
  let rec aux idx frst_part = function
    | [] -> frst_part, []
    | hd :: tl as lst ->
      if idx = 0 then frst_part, lst else aux (idx - 1) (frst_part @ [ hd ]) tl
  in
  aux n [] list
;;
...
let rotate list n =
  let rec aux idx out n = function
    | [] -> out
    | hd :: tl ->
      if idx < n then aux (idx + 1) out n tl else aux (idx + 1) (out @ [ hd ]) n tl
  in
  aux 0 [] n list
;;
...
let rec insert_at elem idx = function
  | [] -> [ elem ]
  | hd :: tl as lst -> if idx = 0 then elem :: lst else hd :: insert_at elem (idx - 1) tl
;;
...
let rand_select list n =
  let rec get out idx = function
    | [] -> raise Exit
    | hd :: tl -> if idx = 0 then hd, out @ tl else get (hd :: out) (idx - 1) tl
  in
...
let phi m =
  let rec aux counter m = function
    | [] -> counter
    | hd :: tl -> if coprime hd m then aux (counter + 1) m tl else aux counter m tl
  in
  aux 0 m (range 1 (m - 1))
;;
...
let factors n =
  let rec aux n d =
    if n = 1 then [] else if n mod d = 0 then d :: aux (n / d) d else aux n (d + 1)
  in
  aux n 2
;;
...
let table variables expr =
  let rec eval value_of_variables = function
    | Var x ->
      List.assoc x value_of_variables (* dunno how to use pretty Jane Street maps :( )*)
    | Not expr -> not (eval value_of_variables expr)
    | And (expr1, expr2) -> eval value_of_variables expr1 && eval value_of_variables expr2
    | Or (expr1, expr2) -> eval value_of_variables expr1 || eval value_of_variables expr2
  in
...
let construct list =
  let rec push tree value =
    ...
    | Node (already_in, l, r) ->
      if value = already_in
      then tree
      else if value > already_in
      then Node (already_in, l, push r value)
